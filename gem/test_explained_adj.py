...
def main():
    ...
    for node_idx in node_idxs:
        ...
        extracted_pred, extracted_p = evaluate_adj(
            node_idx_new,
            sub_feat,
            extracted_adj,
            sub_label,
    ...
    print("{0:<25}{1:<25}{2:<25}{3:<25}".format(
        "org_adj",
        np.mean(org_losses),
        np.mean(org_corrects),
        metrics.f1_score(np.ones(len(org_corrects)), org_corrects)
    ))
    
    print("{0:<25}{1:<25}{2:<25}{3:<25}".format(
        "extracted_adj",
        np.mean(extracted_losses),
        np.mean(extracted_corrects),
        metrics.f1_score(np.ones(len(extracted_corrects)), extracted_corrects)
    ))
    
    print("{0:<25}{1:<25}{2:<25}{3:<25}".format(
        "ours_adj(top_k=%s)" % prog_args.top_k,
        np.mean(ours_losses),
        np.mean(ours_corrects),
        metrics.f1_score(np.ones(len(ours_corrects)), ours_corrects)
    ))

    print("{0:<25}{1:<25}{2:<25}{3:<25}".format(
        "gnnexp_adj(top_k=%s)" % prog_args.top_k,
        np.mean(gnnexp_losses),
        np.mean(gnnexp_corrects),
        metrics.f1_score(np.ones(len(gnnexp_corrects)), gnnexp_corrects)
    ))
    ...
    print("{0:<25}{1:<25}{2:<25}".format(
        "our_ground_truth_odd",
        np.nanmean(our_ground_truth_odd),
        np.nanstd(our_ground_truth_odd)
    ))

    print("{0:<25}{1:<25}{2:<25}".format(
        "gnnexp_ground_truth_odd",
        np.nanmean(gnnexp_ground_truth_odd),
        np.nanstd(gnnexp_ground_truth_odd)
    ))

    print("{0:<25}{1:<25}{2:<25}".format(
        "our_pred_label_odd",
        np.nanmean(our_pred_label_odd),
        np.nanstd(our_pred_label_odd)
    ))

    print("{0:<25}{1:<25}{2:<25}".format(
        "gnnexp_pred_label_odd",
        np.nanmean(gnnexp_pred_label_odd),
        np.nanstd(gnnexp_pred_label_odd)
    ))
