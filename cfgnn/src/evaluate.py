...
parser.add_argument('--path', '-p', required=True, help='Filepath of CF example dataset')
...
if "syn4" in args.path:
	...
	dataset = "syn1"
elif "small_amazon" in args.path:
	dataset = "small_amazon"
	hidden = 128 # 64 for old amazon, 128 for new.
...
with open(f"../data/Eval-sets/{dataset}.pickle", "rb") as file:
...
for i in range(len(df_motif)):
	...
	perturb_nodes_orig_ypred = np.array([dict_ypred_orig[k] for k in perturb_nodes_orig_idx if k in dict_ypred_orig])
...
#* >>> Per label metrics
for label in y_pred_orig.unique().tolist():
	if label == 0:
		continue
	df_current = df.copy(deep=True)
	df_current = df_current[df_current["y_pred_orig"] == label].reset_index(drop=True)
	# Add num edges
	num_edges = []
	for i in df_current.index:
		num_edges.append(sum(sum(df_current["sub_adj"][i])) / 2)
	df_current["num_edges"] = num_edges

	# For accuracy, only look at motif nodes
	df_motif = df_current[df_current["y_pred_orig"] != 0].reset_index(drop=True)
	accuracy = []
	# Get original predictions
	dict_ypred_orig = dict(zip(sorted(np.concatenate((idx_train.numpy(), idx_test.numpy()))), y_pred_orig.numpy()))
	for i in range(len(df_motif)):
	...
	print(f">>> Label {label}")
	print("Num cf examples found: {}/{}".format(len(df_current), len(idx_test)))
	print("Avg fidelity: {}".format(1 - len(df_current) / len(idx_test)))
	print("Average graph distance: {}, std: {}".format(np.mean(df_current["loss_graph_dist"]), np.std(df_current["loss_graph_dist"])))
	print("Average sparsity: {}, std: {}".format(
		np.mean(1 - df_current["loss_graph_dist"] / df_current["num_edges"]), np.std(1 - df_current["loss_graph_dist"] / df_current["num_edges"])
	))
	print("Accuracy", np.mean(df_accuracy["prop_correct"]), np.std(df_accuracy["prop_correct"]))
	print("\n***************************************************************\n")
