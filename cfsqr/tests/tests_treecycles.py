"""Node classification baselines treecycles"""
...
from utils.preprocessing.tree_cycles_preprocessing import \
    tree_cycles_preprocessing
...
for node_id, number_of_cfs in num_dict.items():
    ...
    if label != new_label_dict[node_id]:
...
## * Accurcy
G_dataset = tree_cycles_preprocessing(
    dataset_dir="datasets/Tree_Cycles",
    hop_num=4
)

base_model = GCNNodeTreeCycles(
...
base_model.load_state_dict(
    torch.load("cfgnn_model_weights/gcn_3layer_syn4.pt")
)
base_model.eval()
...
for gid, cf_adj in exp_dict.items():
    ...
    adj = adj * (adj > 0.1) # Threshold defined by cfsqr's authors.
    cfs = adj.nonzero()
    cf_acc = 0
    # if no cf was found
    if cfs.size(0) == 0:
