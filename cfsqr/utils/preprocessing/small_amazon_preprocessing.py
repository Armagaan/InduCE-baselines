...
class SmallAmazonDataset(DGLDataset):
    def __init__(self, adj=None, node_labels=None, edge_labels=None, hop_num=3, feats=None, feat_dim=10, load_path=None):
        super().__init__(name='small_amazon')
        if load_path:
...
def small_amazon_preprocessing(dataset_dir, hop_num=4):
    name = "small_amazon"
    # assign path
    ...
    node_labels = data[2]
    e_labels = data[3]
    G_dataset = SmallAmazonDataset(adj, node_labels, e_labels, hop_num=hop_num, feats=feats, feat_dim=feats.shape[1])
    return G_dataset
